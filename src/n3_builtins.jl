...
# math:logarithm: (base value) math:logarithm result
function _builtin_math_logarithm(subject::Identifier, object::Identifier,
    ...
    items = _resolve_rdf_list(s, graph)
    if items !== nothing && length(items) == 2
        base = _to_number(items[1])
        val = _to_number(items[2])
        (base !== nothing && val !== nothing && base > 0 && val > 0) || return _failure()
        return _bind_or_check(object, _from_number(log(base, val)), bindings)
    end
...
function _builtin_string_replace(subject::Identifier, object::Identifier,
    ...
    if items !== nothing && length(items) == 3
        ...
        result = replace(items[1].lexical, Regex(items[2].lexical) => items[3].lexical)
        return _bind_or_check(object, Literal(result), bindings)
    end
    # Fallback
    o = _resolve(object, bindings)
    (s isa Literal && o isa Literal) || return _failure()
    parts = split(o.lexical, "/"; limit=2)
    length(parts) == 2 || return _failure()
    result = replace(s.lexical, Regex(parts[1]) => parts[2])
...
function _builtin_string_substring(subject::Identifier, object::Identifier,
    ...
    if items !== nothing && length(items) >= 2
        ...
        str = items[1].lexical
        si = max(1, Int(start) + 1)  # 0-based to 1-based
        if length(items) >= 3
...
function _builtin_log_dtlit(subject::Identifier, object::Identifier,
    ...
    if items !== nothing && length(items) == 2
        items[1] isa Literal || return _failure()
        items[2] isa URIRef || return _failure()
...
function _builtin_log_hasPrefix(subject::Identifier, object::Identifier,
    ...
    s = _resolve(subject, bindings)
    o = _resolve(object, bindings)
    (s isa URIRef && o isa Literal) || return _failure()
...
function _builtin_log_skolem(subject::Identifier, object::Identifier,
    ...
    o = _resolve(object, bindings)
    if o isa Variable
        sk = URIRef("urn:skolem:" * replace(string(uuid4()), "-" => ""))
...
function _builtin_crypto_md5(subject::Identifier, object::Identifier,
    ...
    s isa Literal || return _failure()
    # MD5 via SHA module
    h = bytes2hex(SHA.sha1(Vector{UInt8}(s.lexical)))  # fallback to sha1 if no md5
    try
        h = bytes2hex(transcode(UInt8, sprint(io -> write(io, s.lexical))) |> x -> SHA.sha1(x))
    catch end
    _bind_or_check(object, Literal(h), bindings)
end
    ...
    for (i, item) in enumerate(items)
        ...
        if obj_list !== nothing && length(obj_list) == 2
            ...
            if idx_term isa Variable
                b[idx_term] = idx_lit
            elseif idx_term != idx_lit
                ok = false
            end
            if ok
                if val_term isa Variable
                    b[val_term] = item
                elseif val_term != item
                    ok = false
                end
            end
            ok && push!(results, b)
...
function _builtin_list_remove(subject::Identifier, object::Identifier,
    ...
    (list1 !== nothing && list2 !== nothing) || return _failure()
    result = copy(list1)
    for item in list2
        idx = findfirst(==(item), result)
        idx !== nothing && deleteat!(result, idx)
    end
    graph === nothing && return _failure()
...
function _builtin_graph_length(subject::Identifier, object::Identifier,
    ...
    _bind_or_check(object, _from_number(length(s.graph)), bindings)
end
...
function _builtin_graph_member(subject::Identifier, object::Identifier,
    ...
    results = Binding[]
    for t in s.graph
        if object isa Variable
            push!(results, merge(bindings, Dict(object => t)))
        end
    end
    isempty(results) ? _success(bindings) : results
end
...
function _builtin_graph_list(subject::Identifier, object::Identifier,
    ...
    ts = collect(s.graph)
    graph === nothing && return _failure()
    items = Identifier[t for t in ts]
    # This doesn't directly translate — graph:list returns list of triples
    # Skip for now
    _failure()
end
...
function _builtin_time_localTime(subject::Identifier, object::Identifier,
    ...
    _bind_or_check(object, Literal(now_str; datatype=URIRef("http://www.w3.org/2001/XMLSchema#dateTime")), bindings)
end
