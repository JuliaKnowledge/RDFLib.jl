...
author: "Simon Frost"
subtitle: "Modeling Infectious Disease Surveillance and Outbreak Analysis with RDFLib.jl"
format:
  html:
    code-fold: false
  pdf:
...
records, and genomic lineage data — into a coherent knowledge model.
**Semantic web technologies** are well-suited for this: RDF naturally
represents linked entities across domains, SPARQL enables complex
cross-cutting queries, and logical reasoning can infer transmission
chains and risk factors.
...
1. **OWL ontology** — pathogens, hosts, clinical features, interventions
...
6. **Datalog reasoning** — transmission inference and risk classification
...
presentations, diagnostic tests, locations, interventions, and
contact events.
...
Using property paths we can trace multi-step contact chains to
...
supporting **math builtins**, **string operations**, and
**backward chaining**. This makes it ideal for epidemiological
inference rules that involve thresholds, date arithmetic, and
complex classification logic.

### 8a. Outbreak classification rules

We classify cases using forward-chaining N3 rules with `math:`
builtins to evaluate thresholds.
...
Using N3 math builtins to compute epidemiological metrics from
pairs of linked cases.
...
attributes with epidemiological criteria to flag cases needing
...
We model the probability of outbreak outcomes given interventions
using ProbLog. The model captures: vaccination effectiveness,
quarantine compliance, and secondary attack rates.
