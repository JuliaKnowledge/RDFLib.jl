...
Notation3 (N3) extends RDF with formulas (quoted graphs), variables, and implication rules. RDFLib.jl includes a full N3 reasoner based on the Euler Abstract Machine (EAM) with support for 100+ built-in predicates covering math, string, list, logic, cryptography, and time operations.
...
Rules in N3 use `=>` (log:implies) to express "if antecedent then consequent":
...
Rules can chain — the conclusion of one rule feeds the antecedent of another:
...
::: {.callout-warning}
The N3 cryptographic builtins (`crypto:md5`, `crypto:sha`) require literals
as triple subjects, which RDFLib.jl currently rejects via strict RDF
semantics enforcement (`Triple subjects cannot be literals`). The cell
below is shown for reference but is not executed pending a parser update
that supports literal subjects inside N3 formulas.
:::
...
The `log:conclusion` builtin runs the reasoner on a formula — enabling meta-reasoning:
